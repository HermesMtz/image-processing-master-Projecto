var MathImg = /** @class */ (function () {
  ...
  MathImg.relativeBrightness = function (img) {
    ...
    for (var i = 0; i < img.getHeight(); i++) {
      for (var j = 0; j < img.getWidth(); j++) {
        ...
        sal[0][i][j] =
          Math.sqrt(
            Math.pow(arrImage[0][i][j], 2) * 0.299 +
              Math.pow(arrImage[1][i][j], 2) * 0.587 +
              Math.pow(arrImage[2][i][j], 2) * 0.114
          ) / 100.0;
        sal[1][i][j] =
          "rgb(" +
          arrImage[0][i][j] +
  ...
  MathImg.escalar = function (img, factor) {
    ...
    var arrImage = img.getArrayImg();
    //variable donde guardamos la salida
    var newHeight = Math.floor(img.getHeight() * factor),
