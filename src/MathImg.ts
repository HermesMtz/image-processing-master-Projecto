...
export class MathImg {
  ...
  public static toGray(img: ImageType): number[][][] {
    ...
    for (let i = 0; i < img.getHeight(); i++) {
      for (let j = 0; j < img.getWidth(); j++) {
        //0.299 + 0.587G + 0.114B.
        prom =
          0.299 * arrImage[0][i][j] +
          0.587 * arrImage[1][i][j] +
          0.114 * arrImage[2][i][j];
        sal[0][i][j] = prom;
        sal[1][i][j] = prom;
        sal[2][i][j] = prom;
      }
  ...
  //Este es el efecto TricolorHorizontal
  public static toTricolorHorizontal(img: ImageType): number[][][] {
    ...
    var sal = this.initArray(img.getWidth(), img.getHeight());
    let inicio = 0,
      termino = img.getHeight() / 3; // Se cambio a img.getHeight()

    for (let j = 0; j < termino; j++) {
      //Se cambio el orden de la i y la j
    ...
    inicio = termino;
    termino = img.getHeight(); // Se cambio a img.getHeight()
  ...
  public static toDesfaceX(img: ImageType, des: number): number[][][] {
    ...
    //variable donde guardamos la salida
    var sal: number[][][] = this.initArray(img.getWidth(), img.getHeight());
    var fila = arrImage[0].length,
      cols = arrImage[0][0].length;
  ...
  public static toUmbral2limites(
    img: ImageType,
    rangos: number[]
  ): number[][][] {
    //variable que guarda el arreglo 3d de la imagen de color
  ...
  public static cambioFTransferencia(
    img: ImageType,
    factores: number[]
  ): number[][][] {
    var arrImage: number[][][] = img.getArrayImg();
  ...
  public static relativeBrightness(img: ImageType): number[][][] {
    ...
    for (let i = 0; i < img.getHeight(); i++) {
      for (let j = 0; j < img.getWidth(); j++) {
        ...
        sal[0][i][j] =
          Math.sqrt(
            arrImage[0][i][j] ** 2 * 0.299 +
              arrImage[1][i][j] ** 2 * 0.587 +
              arrImage[2][i][j] ** 2 * 0.114
          ) / 100.0;
        sal[1][i][j] =
          "rgb(" +
          arrImage[0][i][j] +
  ...
  public static marcaAguaCentro(
    ...
    for (let i = 0; i < heightsmall; i++) {
      for (let j = 0; j < widthsmall; j++) {
        sal[0][midH - midsmallH + i][midW - midsmallW + j] +=
          arrImage2[0][i][j] * porc;
        sal[1][midH - midsmallH + i][midW - midsmallW + j] +=
          arrImage2[1][i][j] * porc;
        sal[2][midH - midsmallH + i][midW - midsmallW + j] +=
          arrImage2[2][i][j] * porc;
      }
    }
        ...
        for (let i = 0; i < heightsmall; i++) {
          for (let j = 0; j < widthsmall; j++) {
            if (i + w * heightsmall < height && j + v * widthsmall < width) {
              sal[0][i + w * heightsmall][j + v * widthsmall] =
                arrImage[0][i + w * heightsmall][j + v * widthsmall] +
                arrImage2[0][i][j] * porc;
              sal[1][i + w * heightsmall][j + v * widthsmall] =
                arrImage[1][i + w * heightsmall][j + v * widthsmall] +
                arrImage2[1][i][j] * porc;
              sal[2][i + w * heightsmall][j + v * widthsmall] =
                arrImage[2][i + w * heightsmall][j + v * widthsmall] +
                arrImage2[2][i][j] * porc;
            }
            //else
            // break;
    ...
    for (let i = 0; i < img.getHeight(); i++) {
      for (let j = 0; j < img.getWidth(); j++) {
        ...
        hue =
          ...
          Math.acos(
            (0.5 *
              (arrImage[0][i][j] -
                arrImage[1][i][j] +
                (arrImage[0][i][j] - arrImage[2][i][j]))) /
              Math.sqrt(
                Math.pow(arrImage[0][i][j] - arrImage[1][i][j], 2) +
                  (arrImage[0][i][j] - arrImage[2][i][j]) *
                    (arrImage[1][i][j] - arrImage[2][i][j])
              )
          );
        ...
        sal[1][i][j] =
          1 -
          (3 *
            Math.min(arrImage[0][i][j], arrImage[1][i][j], arrImage[2][i][j])) /
            (arrImage[0][i][j] + arrImage[1][i][j] + arrImage[2][i][j]);
        sal[2][i][j] =
          (arrImage[0][i][j] + arrImage[1][i][j] + arrImage[2][i][j]) / 3;
      }
    }
    return sal;
  }
      ...
      for (let j = 0; j < width; j++) {
        ...
        if (arrImage[0][i][j] >= 0 && arrImage[0][i][j] < 120) {
          sal[0][i][j] =
            arrImage[2][i][j] *
            (1 +
              (arrImage[1][i][j] * Math.cos(this.toRad(arrImage[0][i][j]))) /
          ...
          sal[1][i][j] =
            ...
            (1 +
              (arrImage[1][i][j] *
                Math.cos(this.toRad(arrImage[0][i][j] - 120))) /
          ...
          sal[2][i][j] =
            ...
            (1 +
              (arrImage[1][i][j] *
                Math.cos(this.toRad(arrImage[0][i][j] - 240))) /
  ...
  public static escalar(img: ImageType, factor: number): number[][][] {
    ...
    var arrImage = img.getArrayImg();
    //variable donde guardamos la salida
    let newHeight = Math.floor(img.getHeight() * factor),
      newWitdh = Math.floor(img.getWidth() * factor);
    var sal = this.initArray(newWitdh, newHeight);

    for (let i = 0; i < newHeight; i++) {
      for (let j = 0; j < newWitdh; j++) {
        sal[0][i][j] =
          arrImage[0][Math.floor(i / factor)][Math.floor(j / factor)];
        sal[1][i][j] =
          arrImage[1][Math.floor(i / factor)][Math.floor(j / factor)];
        sal[2][i][j] =
          arrImage[2][Math.floor(i / factor)][Math.floor(j / factor)];
      }
    }
    /*
    for (let i = 0; i < img.getHeight(); i++) {
